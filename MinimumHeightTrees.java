...
public class MinimumHeightTrees {
	...
	public static List<Integer> findMinHeightTrees(int n, int[][] edges) {
		...
		for (int node = 0; node < n; node++) {
			...
			height = buildNewTree(root, edges, n, height, visited);
	...
	private static int buildNewTree(int root, int[][] edges, int n, int height, boolean[] visited) {
		if (edges.length == 0)
			return height;
		...
		if (children.size() != 0) {
			...
			for (int child : children) {
				int childHeight = buildNewTree(child, edges, n, height, visited);
